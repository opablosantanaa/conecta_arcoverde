...
export default function Usuarios() {
  ...
  const desativarMutation = useMutation({
    ...
    const resetMutation = useMutation({
        ...
        onError: (error: any) => {
            ...
            alert('Erro ao resetar senha: ' + msg);
        },
    });
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      {modalReset && (
        <Modal title="Resetar Senha" onClose={() => { setModalReset(null); setNovaSenha('); setConfirmarSenha('); }}>
          <div className="space-y-4">
      ...
      <Input
        ...
        hint={confirmarSenha && novaSenha !== confirmarSenha ? "As senhas nao conferem" : ""}
      />
            <div className="flex gap-2">
<Button onClick={() => { if (modalReset && novaSenha.length >= 8 && novaSenha === confirmarSenha) { resetMutation.mutate({ id: modalReset, novaSenha }); } }} disabled={novaSenha.length < 8 || novaSenha !== confirmarSenha || resetMutation.isPending} isLoading={resetMutation.isPending}>Confirmar Reset</Button>
